...
/**
 * Injects content into partials, supporting data attributes.
 *
...
async function injectPartials(filePath, options = {}) {
  const { type, data = {}, publicDir = 'public' } = options;
 ...
 * @param {string} options.publicDir - Base public directory
 * @param {string} options.content - Content to inject into the partial (for parts)
 * @param {Record<string, string>} options.data - Data attributes for interpolation
...
async function loadAndInjectPartial(name, options = {}) {
  const { type, viewsDir = 'views', publicDir = 'public', content, data = {} } = options;
