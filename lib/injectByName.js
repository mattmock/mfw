 ...
 */
function getCacheKey(name, type, data = {}) {
  return `${type}:${name}:${JSON.stringify(data)}`;
}
 ...
 * @param {Object} options.data - Data attributes for interpolation
...
async function injectByName(filePathOrHtml, options = {}) {
  ...
  root.querySelectorAll('struct').forEach(struct => {
    ...
    const attrs = struct.rawAttrs;
    const dataAttrRegex = /data-([^=]+)="([^"]+)"/g;
    let match;
    while ((match = dataAttrRegex.exec(attrs)) !== null) {
      structData[match[1]] = match[2];
    }

    // Check cache
    const cacheKey = getCacheKey(structName, 'struct', structData);
    ...
    structPromises.push(
      loadAndInjectPartial(structName, {
        ...
        structContent.querySelectorAll('part').forEach(part => {
          ...
          // Check cache
          const partCacheKey = getCacheKey(partName, 'part', structData);
  ...
  await Promise.all(structPromises);
